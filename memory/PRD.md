...
- Digital KYC — bug fix + camera liveliness (2026-08, `KycVerification.tsx`): **Fixed** the EKYC getting stuck at step 3 ("Running final checks…" forever, wizard Continue disabled) — VKYC worked because it completes via a plain event-handler timeout, but EKYC completed via a `useEffect` whose cleanup cancelled the pending timeout (on `compliance` flip / dep re-run / StrictMode) so `onVerified()` never fired. Robust fix: the completion timer is stored in `complianceTimerRef` and is **only cleared on unmount** (never on effect re-run), with a one-shot `complianceStartedRef` guard; `compliance` removed from deps. Also replaced the selfie capture with a real **camera motion-based liveliness check** (`LivenessCheck` component): random challenge (blink/turn/smile/nod), on-device frame-diff motion score vs threshold, captures the highest-motion frame; on pass → `liveness="done"`. Demo-safe fallbacks so the flow never dead-ends without hardware: `data-testid="liveness-simulate"` ("Use simulated liveliness (demo)") and `data-testid="ekyc-location-simulate"` ("Use demo location") when the camera/geolocation are unavailable. Verified end-to-end by frontend testing agent (KYC completes, Continue enables, advances to step 4).
